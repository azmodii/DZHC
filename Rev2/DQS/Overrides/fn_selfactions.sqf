...
if ((speed player <= 1) && _isAirdrop && _canDo && (player distance cursorTarget < 4)) then {
	if (s_player_Airdrop < 0) then {
		s_player_Airdrop = player addAction [("<t color=""#0096ff"">" + ("Fix Notebook (Airdrop)") +"</t>"),"Mods\airdrop\Airdropp.sqf",cursorTarget, 0, false, true, "",""];
	};
} else {
...
// IT07's Server Menu script
    if((speed player <= 1) && _canDo) then {
        if (s_player_servermenu < 0) then {
			s_player_servermenu = player addAction [("<t color=""#FFFFFF"">" + ("Server Rules") +"</t>"),"Mods\servermenu\spawn_rules.sqf","",-1,false,false,"", ""];
...
// ------------------------------------------------------------------------Krixes Self Bloodbag Start------------------------------------------------------------------------
	...
	if((speed player <= 1) && hasBagItem && _canDo) then {
		if (s_player_selfBloodbag < 0) then {
			s_player_selfBloodbag = player addaction[("<t color=""#c70000"">" + ("Self Bloodbag") +"</t>"),"Mods\SelfBB\player_selfbloodbag.sqf","",5,false,true,"", ""];
		};
	} else {
...
if (!isNull cursorTarget and !_inVehicle and !_isPZombie and (player distance cursorTarget < _allowedDistance) and _canDo) then {	//Has some kind of target
	...
	if ((_isDeployed) && ((player distance cursorTarget) < 5)) then {
			...
			if (s_player_packOBJ < 0) then {
					s_player_packOBJ = player addaction [("<t color=""#00FF04"">" + ("Pack "+typeOf _dObj+"") +"</t>"),"Mods\Deploy\pack.sqf",_dObj,6,false,true,"", ""]
			};
	} else {
