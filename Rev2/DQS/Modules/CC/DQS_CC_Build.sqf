...
if (DQS_CC_AdminFastBuild) then {_isBuildAdmin = (getPlayerUID player) in DQS_AdminList;} else {_isBuildAdmin = false;};
...
if (!_typeIsString) then {
	_classname = _dqsArr select 0;
	_classnametmp = _classname;
	_require = _dqsArr select 1;
};
...
_hasbuilditem = true;
_needbuilditem = _dqsArr select 2;
diag_log _dqsArr;
diag_log (typeName _needBuildItem);
if (!_isBuildAdmin) then {
	...
	// DQS 1/2 Edits
	if (!_typeIsString && _hasRequiredTools && !_isBuildAdmin) then {
		...
		_oName = "";
		{
			...
			_loop = 0;
			if (_magCount < _magNeeded) then {
				_hasbuilditems = false;
				_oName = getText (configFile >> "cfgMagazines" >> _magType >> "displayName");
				if (_loop > 0) then {_separator = ", ";} else {_separator = "";};
				_stringFailed = _stringFailed + format["%3%1 x %2",_magNeeded,_oName,_separator];
				_loop = _loop + 1;
			};
		} forEach _needbuilditem;
		if(!_hasbuilditems) then {_missingB = _stringFailed; }
	};
};
// Message If missing
...
if (!_typeIsString && !_hasbuilditems) exitWith {DZE_ActionInProgress = false; cutText [format[(localize "str_player_31"),_missingB,"build"] , "PLAIN DOWN"]; };
if (_typeIsString && !_hasbuilditems) exitWith {DZE_ActionInProgress = false; cutText [format[(localize "str_player_31"),_missingB,"build"] , "PLAIN DOWN"]; };


//Proceed after item check
if (_hasRequiredTools && _hasbuilditems) then {
	...
	if(!_cancel) then {
		...
		diag_log "Proceed OK";
			...
			if (!_isBuildAdmin) then {
				...
				if (!_typeIsString) then {
					_removed_total = 0;	
					diag_log _needbuilditem;
					{
						_magType = _x select 0;
						_magNeeded = _x select 1;
						for "_k" from 0 to (_magNeeded - 1) do
						{
							diag_log format["_k %1 - _magType %2"];
							_removed = ([player,_magType] call BIS_fnc_invRemove);
							_temp_removed_array = _temp_removed_array + [_magType];
							diag_log _removed;
						};
						_removed_total = _removed_total + _removed;
					} forEach _needbuilditem;
				};
				diag_log _removed_total;
				// DQS EDIT 2/2 END
			} else {
